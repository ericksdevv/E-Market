...
export class AddressesService {
  ...
  async create(userId: number, data: AddressDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      if (data.isDefault || count === 0) await tx.address.updateMany({ where: { userId }, data: { isDefault: false } });
      return tx.address.create({ data: { ...data, userId, state: data.state.toUpperCase(), isDefault: data.isDefault || count === 0 } });
    });
  }
  ...
  async update(userId: number, id: number, data: AddressDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      return tx.address.update({ where: { id }, data: { ...data, state: data.state.toUpperCase() } });
    });
  }
