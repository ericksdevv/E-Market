...
export class AdminController {
  ...
  @Patch('orders/:id/status') updateOrder(
    ...
    return this.admin.updateOrder(Number(id), status);
  }
  @Get('clients') clients() {
    return this.admin.clients();
  }
  @Patch('clients/:id/status') toggleClient(
    @Param('id') id: string,
    @Body('isActive') active: boolean,
  ) {
    return this.admin.toggleClient(Number(id), active);
  }
  @Post('stock/:productId') stock(
    @Param('productId') productId: string,
    @Body() body: { quantity: number; reason?: string },
  ) {
    return this.admin.adjustStock(
      Number(productId),
  ...
  @Post('products') createProduct(
    @Body() body: Prisma.ProductUncheckedCreateInput,
  ) {
  ...
  @Patch('products/:id') updateProduct(
    ...
    return this.admin.updateProduct(Number(id), body);
  }
}
